...
namespace 期末專題_旅遊網.ViewModels.Models
{
    public class tADFactory
    {
        ...
        string savePath = "E:/資策會/003專題/我的檔案/期末專題/旅遊網/期末專題_旅遊網/期末專題_旅遊網/img/ADImage/";
        //新增圖片
        ...
        //廣告業面3個參數+搜尋式
        IQueryable<tAdvertising> SelectAdShow(string date)
        {            
            var q = from result in db.tAdvertising
                    where result.cADStatus == true &&
                          result.cADGroup == "1" &&
                          result.cADActionDate.Contains(date)
                    select result;

            return q;
        }
        //1.廣告數量
        public int AdCount(string date)
        {
            //圖片總數            
            return SelectAdShow(date).Count();
        }
        //2.廣告圖片名
        public string[] AdName(string date)
        {
            string[] adFileName;
            //取出圖片名
            if (AdCount(date) == 0)
            ...
            {
                adFileName = new string[AdCount(date)];
                int adFileNameCount = 0;
                foreach (var i in SelectAdShow(date))
                {
        ...
        //3.廣告URL
        public string[] AdURL(string date)
        {
            //取出圖片URL
            string[] adURL;
            if (AdCount(date) == 0)
        ...
        public IQueryable<tAdvertising> QueryBySNAll(int firmSn)
        {
            var q = from result in db.tAdvertising
                    where result.cFirmSN == firmSn.ToString()
                    select result;
